...
def process_official_directory(selected_jpgs=None):
    ...
    # 480P：Whisper ASR（剪片依據）+ Grok 4.5 翻譯；失敗改用 Step 3.7 Flash；關閉 enhance
    ...
    print(
        ...
        f"（代理 → Whisper + Grok 4.5（失敗改 Step 3.7 Flash）→ 480P 剪片，不 enhance）\n"
    )
    success_count = 0
    failed_count = 0
...
def run_download_process(
    ...
    try:
        ...
        if not retry_subtitles and selected_high_jpgs:
            ...
            print(
                " [階段 2/3] 03_videos"
                "（Whisper+Grok 4.5（失敗改 Step 3.7 Flash）→480P 剪片，不 enhance）"
            )
            print("==================================================")
        ...
        # 【階段 3】精選：1080P + MOSS + Grok 4.5 + enhance → 06_good
        if not retry_subtitles and not repair_over_1080 and chosen_items:
            print("\n==================================================")
            print(
                " [階段 3/3] 05_chosen"
                "（1080P + MOSS + Grok 4.5（失敗改 Step 3.7 Flash）+ enhance → 06_good）"
            )
            print("==================================================")
            import chosen_pipeline
