...
function profileSummary(profile) {
  ...
  return [
    ...
    profile.mode === "video" ? `≤${options.video_high_quality_max_seconds}s 來源最高` :
      (options.vocal_separator === "roformer" ? "RoFormer" : "Demucs"),
...
function emptyProfile() {
  ...
  return {
    ...
    options: {
      ...
      video_high_quality_max_seconds: 240, vocal_separator: "demucs",
...
function profileRevision(profile) {
  ...
  return JSON.stringify({
    ...
    color: profile.color || "#d6ff3f",
    inbox_dir: profile.inbox_dir || "",
    output_dir: profile.output_dir || "",
    grid_backup_dir: profile.grid_backup_dir || "",
    route_mode: profile.route_mode || "copy",
...
function collectProfileForm() {
  const options = {};
  const current = state.profiles.find(item => item.id === $("#profileId").value)
    || state.profileDraft;
  $$("[data-option]", $("#profileForm")).forEach(input => {
...
function updateProfileModeFields() {
  ...
  threePhase.disabled = !["shorts", "video", "chosen"].includes(mode);
...
async function saveProfile(event) {
  ...
  if (error) return;
  try {
    const fresh = (await api("/api/settings")).settings;
    const profiles = [...fresh.profiles];
    const index = profiles.findIndex(item => item.id === profile.id);
    if (
      index >= 0
      && state.profileDraft?.id === profile.id
      && profileRevision(profiles[index]) !== profileRevision(state.profileDraft)
    ) {
      throw new Error("此 Profile 已由 JSON 或其他頁面更新，請關閉後重新開啟再修改。");
    }
    if (index >= 0) profiles[index] = profile;
    else profiles.push(profile);
    const payload = await api("/api/settings", {
      method: "POST",
      body: JSON.stringify({ settings: { profiles } }),
    });
...
async function deleteCurrentProfile() {
  ...
  const fresh = (await api("/api/settings")).settings;
  const profiles = fresh.profiles.filter(profile => profile.id !== id);
  const payload = await api("/api/settings", {
    method: "POST",
    body: JSON.stringify({ settings: { profiles } }),
  });
