...
class RedTubeAdapter(SiteAdapter):
    name = "redtube"
    domains = ("redtube.com",)

    def search_url(self, keyword: str) -> str:
        q = urllib.parse.quote(keyword.strip(), safe="")
        return f"https://www.redtube.com/?search={q}"
    ...
    def extract_list_urls_from_html(self, page_url: str) -> list[str]:
        ...
        if not ids:
            ids = re.findall(r'href="/(?:[^"]*/)?(\d{6,})"', html)
        seen: set[str] = set()
        urls: list[str] = []
        for vid in ids:
            if vid in seen:
                continue
            seen.add(vid)
            urls.append(f"https://www.redtube.com/{vid}")
        return urls
