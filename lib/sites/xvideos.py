"""XVideos site adapter."""

from __future__ import annotations

import re
import urllib.parse

from sites.base import SiteAdapter


class XVideosAdapter(SiteAdapter):
    name = "xvideos"
    domains = ("xvideos.com", "xvideos.es")

    def search_url(self, keyword: str) -> str:
        q = urllib.parse.quote(keyword.strip(), safe="")
        return f"https://www.xvideos.com/?k={q}"

    def get_start_page(self, url: str) -> int:
        m = re.search(r"[?&]p=(\d+)", url or "")
        if m:
            return int(m.group(1)) + 1  # site uses 0-based p=
        return super().get_start_page(url)

    def build_page_url(self, url: str, page_num: int) -> str:
        # XVideos search uses p=0, p=1, ...
        p = max(0, page_num - 1)
        if re.search(r"[?&]p=\d+", url):
            return re.sub(r"([?&]p=)\d+", rf"\g<1>{p}", url)
        if page_num <= 1:
            return url
        join = "&" if "?" in url else "?"
        return f"{url}{join}p={p}"

    def is_single_video_url(self, url: str) -> bool:
        path = urllib.parse.urlsplit(url).path or ""
        # Legacy /video12345/title and newer /video.abc123hash/title
        return bool(re.search(r"/video(?:\d+|\.[a-z0-9]+)/", path, re.I))

    def extract_list_urls_from_html(self, page_url: str) -> list[str]:
        try:
            html = self.fetch_html(page_url)
        except Exception:
            return []
        paths = re.findall(r'href="(/video(?:\d+|\.[a-z0-9]+)/[^"]+)"', html, re.I)
        if not paths:
            paths = re.findall(r'(/video\.[a-z0-9]+/[^"\'\s<>]+)', html, re.I)
        if not paths:
            return []
        seen: set[str] = set()
        urls: list[str] = []
        base = f"{urllib.parse.urlsplit(page_url).scheme}://{urllib.parse.urlsplit(page_url).netloc}"
        for path in paths:
            path = path.split("#")[0]
            if path in seen:
                continue
            seen.add(path)
            urls.append(self.absolute_url(base + "/", path))
        return urls
