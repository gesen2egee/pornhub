...
3. 高畫質切塊在全管線固定維持最多 8 個並行下載請求，新請求每秒錯開啟動；任一切塊完成就立刻補送下一段。每段會先多抓 5 秒前置緩衝，再由本機精確重編碼，讓成品從新的關鍵影格開始，並完整解碼驗證；個別切塊失敗時會改為單線補下載，最後同步進行音訊判斷／`enhance` 與 0.08 秒影音 crossfade、retime 字幕。
